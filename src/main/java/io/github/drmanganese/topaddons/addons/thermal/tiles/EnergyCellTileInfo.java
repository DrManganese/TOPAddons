...
public class EnergyCellTileInfo implements ITileInfo<EnergyCellTile> {
    ...
    @Override
    public void addProbeInfo(ProbeMode probeMode, IProbeInfo probeInfo, Player player, Level world, BlockState blockState, IProbeHitData hitData, @Nonnull EnergyCellTile tile) {
    ...
    private static void ioIcon(IProbeInfo probeInfo, int amount, ResourceLocation icon) {
        probeInfo.horizontal(ICON_LAYOUT_STYLE)
            ...
            .text(CompoundText.create().label(new TextComponent(String.format("%d RF/t", amount))));
    }
}
