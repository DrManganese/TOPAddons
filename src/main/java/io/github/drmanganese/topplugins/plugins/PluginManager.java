...
public class PluginManager {
    ...
    public static void preInit(FMLPreInitializationEvent event) {
        ...
        TOPPlugins.LOGGER.info("Found {} plugin candidates.", asmDataSet.size());
        ...
        for (ASMDataTable.ASMData asmData : asmDataSet) {
            ...
            if (dependency == null || dependency.isEmpty()) {
                ...
                if (Loader.isModLoaded(dependency)) {
                    ...
                    if (success) {
                        TOPPlugins.LOGGER.info("Created plugin {} with {} helmets.", fancyName, numHelmets);
                    } else {
    ...
    private static int addHelmets(ITOPPlugin plugin, String fancyName) {
        int ret = 0;
        if (plugin.hasHelmets()) {
            for (Class<? extends ItemArmorProbed> clazz : plugin.getHelmets()) {
                try {
